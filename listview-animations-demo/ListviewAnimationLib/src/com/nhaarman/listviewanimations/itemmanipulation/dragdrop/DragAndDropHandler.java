...
import com.demon.listviewanimationtest.itemmanipulation.DynamicListViewActivity;
 ...
 * <p/>
 * Users of this class must call {@link #onTouchEvent(MotionEvent)} and {@link #dispatchDraw(Canvas)} on the right moments.
 */
@TargetApi(14)
public class DragAndDropHandler implements TouchEventHandler {
    ...
    /**
     * The {@link ListAdapter} that is assigned. Also implements {@link Swappable}.
     */
    @Nullable
     ...
     * This value is {@value AdapterView#INVALID_POSITION} if and only if the user is not dragging.
     */
     ...
     * @throws IllegalArgumentException if the adapter does not implement {@link Swappable}.
     */
     ...
     *
     * @param event the {@link MotionEvent} that was triggered.
     ...
     *
     * @return the position of the item in the list, or {@link AdapterView#INVALID_POSITION} if the {@code View} corresponding to the id was not found.
     */
    ...
    private void switchIfNecessary() {
        ...
        long aboveItemId = position - 1 - mWrapper.getHeaderViewsCount() >= 0
     ...
     * A distinction is made between API levels because {@link AbsListView.OnScrollListener#onScroll(AbsListView, int, int,
    ...
    private class KitKatSwitchViewAnimator implements SwitchViewAnimator {
        ...
        public void animateSwitchView(final long switchId, final float translationY) {
            assert mMobileView != null;
            mWrapper.getListView().getViewTreeObserver().addOnPreDrawListener(
                    new AnimateSwitchViewOnPreDrawListener(mMobileView, switchId, translationY));
     ...
     * <p/>
     * The {@link #onScroll(AbsListView, int, int, int)} method then takes over the functionality {@link #handleMoveEvent(MotionEvent)} provides.
     */
    ...
    private class SettleHoverDrawableAnimatorListener extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener {
        ...
        private SettleHoverDrawableAnimatorListener(@NonNull final HoverDrawable animatingHoverDrawable,
                                                    @NonNull final View animatingMobileView) {
